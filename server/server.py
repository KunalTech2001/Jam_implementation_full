...
def run_jam_preimages():
    ...
    try:
        if not os.path.exists(jam_preimages_script):
            logger.warning(f"jam-preimages script not found at {jam_preimages_script}")
            return False, "jam-preimages script not found"
            
        result = subprocess.run(
            ["python3", jam_preimages_script],
            cwd=os.path.dirname(jam_preimages_script),  # Run from the script's directory
            capture_output=True,
            text=True,
            check=True
        )
        logger.info(f"jam-preimages component executed successfully. Output: {result.stdout}")
        return True, result.stdout
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to run jam-preimages component: {e.stderr}")
        return False, e.stderr
    except Exception as e:
        logger.error(f"Unexpected error while running jam-preimages component: {str(e)}")
        return False, str(e)
...
async def process_block(request: BlockProcessRequest):
    ...
    try:
        ...
        try:
            ...
            # Still try to run jam-preimages even if jam_history failed
            try:
                jam_preimages_success, jam_preimages_output = run_jam_preimages()
                if not jam_preimages_success:
                    logger.warning(f"jam-preimages execution completed with warnings: {jam_preimages_output}")
            except Exception as preimages_error:
                logger.error(f"Failed to run jam-preimages after error: {str(preimages_error)}")

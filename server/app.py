...
async def startup_event():
    ...
    #     # 🔹 Start Jam-preimages/main.py as a background process
    preimage_path = os.path.join(script_dir, "..", "Jam-preimages", "main.py")
    try:
        subprocess.Popen(["python3", preimage_path])
        logger.info("Jam-preimages/main.py started successfully")
    except Exception as e:
        logger.error(f"Failed to start Jam-preimages/main.py: {str(e)}")

    # 🔹 Start Jam-history/main.py as a background process
    history_path = os.path.join(script_dir, "..", "Jam-history", "test.py")
    try:
        subprocess.Popen(["python3", history_path])
        logger.info("Jam-history/test.py started successfully")
    except Exception as e:
        logger.error(f"Failed to start Jam-history/test.py: {str(e)}")
